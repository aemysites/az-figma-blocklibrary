...
const setupObservers = () => {
  const mutatingBlocks = document.querySelectorAll('div.cards, div.carousel, div.accordion, div.cards-teaser');
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      if (mutation.type === 'childList' && mutation.target.tagName === 'DIV') {
        ...
        const type = mutation.target.classList.contains('cards-teaser-card-image') ? 'cards-image' : mutation.target.attributes['data-aue-component']?.value;
        ...
        switch (type) {
          ...
          case 'cards-image':
            ...
            if (mutation.target.classList.contains('cards-teaser-card-image')) {
